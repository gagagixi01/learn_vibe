...
- 产品负责人：把范围收在一个小函数里，只定义标题空白整理，不扩展到 Web、数据库或复杂校验。
- 测试者：确认先看到正确的 Red，再看到 `pytest` 进入 Green，并检查新增测试仍在规格范围内。
- 学习者：能用自己的话讲清 `spec.md`、`test_task_title.py`、`task_title.py` 和 `review_note.md` 分别解决什么问题。
...
- 你创建并激活 `.venv`，安装了 `pytest`，并确认 `pytest --version` 可以显示版本号。
- 你已经在该目录内启动过 `Claude Code CLI`，并说明本节只练小函数。
- 你写出 `spec.md`，并能说清本次做什么、不做什么。
- 你先运行过失败测试，再写最小实现让测试通过。
- `pytest test_task_title.py` 至少让 3 个基础测试全部通过；如果有额外测试，它们也仍在 `spec.md` 范围内并且全部通过。
- 你写了 `review_note.md`，能解释 SDD、TDD、小步迭代和 Review。
- 你知道下一节 `02` 才会创建正式全栈项目目录 `task-vibe/`。

需要停下处理：

- 如果你跳过失败测试，直接复制最终代码，先回到 Red 阶段重跑。
- 如果你把 FastAPI、SQLite、Jinja2 或 `task-vibe/` 提前引入本节，先收回到小函数实验范围。
- 如果你只看到测试通过，却讲不清 `spec.md`、`test_task_title.py`、`task_title.py`、`review_note.md` 各自作用，先回到文件重新复述。
