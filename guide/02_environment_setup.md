...
```
![[Pasted image 20260606211139.png]]
...
- 确认当前工作目录是 `task-vibe/`。
- 知道 `vibecoding-method-lab/.venv` 和 `task-vibe/.venv` 是两个不同环境。
- 已经创建并激活 `task-vibe/.venv`。
- 已经安装 `fastapi`、`uvicorn`、`jinja2`、`pytest`、`httpx2`、`python-multipart`。
- 已经确认 `python -m pip show fastapi pytest httpx2 python-multipart` 能显示包信息。
- 在环境准备完成后才启动 `Claude Code CLI`。
- Claude 创建 `main.py` 后，我已经退出 Claude 回到普通终端，或新开终端，并重新激活 `task-vibe/.venv`。
- 我使用 `python -m uvicorn main:app --reload` 启动服务。
- 我已经成功启动最小 FastAPI 服务。
- 我已经看到 `curl http://127.0.0.1:8000/` 返回 `{"message":"Task Vibe is running"}`。
- 我知道下一节 `03` 才开始正式写 `SDD` 项目规格。
