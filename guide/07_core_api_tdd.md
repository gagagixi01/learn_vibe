...
![[Pasted image 20260607115108.png]]
常见失败可以这样读：
...
检查Cluade code对 `main.py` 的修改，先确认已经导入数据库工具，并在应用启动时初始化表。下面的导入和初始化代码只用来对照结构，不要求完全一致。
...
![[Pasted image 20260607120330.png]]
...
判断标准：这一条测试确实验证完成状态会反向切换，并且它通过。这里需要特别关注，main.py代码内对于toggle的判断逻辑应该与SQLlite内的值类型保持一致，用整数进行测试，如果大模型用bool来测试要大模型提示词进行修正。
```bash
...
- 你已经亲眼看到过至少一次 Claude Code 运行出来的“先失败，再通过”的测试过程。
