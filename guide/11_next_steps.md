...
终端用于查看当前项目状态、运行测试、启动服务和检查文件。`Claude Code CLI` 用于帮助收敛扩展范围、写 mini spec、写第一条测试、运行测试、做最小实现和 final review；每一步都要限制范围，不要让 Claude 一次性大改项目。全课统一工具边界见 [AGENT.md](/Users/walter/Walter_Study_Wiki/coding_with_ai/learn_vibe/AGENT.md)。
...
- 我已经完成核心路线 `00-10`。
- 我已经在 `11` 走完一轮小扩展闭环。
- 我已经写出 `docs/extension-spec.md`。
- 我已经写出并运行第一条扩展测试，并看到它先失败再通过。
- 我已经完成最小实现、最终验证和 `docs/extension-review.md`。
- 我知道以后每一轮练习，仍然要遵守 `SDD + 轻量 TDD + 小步迭代 + Review`。
- 我知道这套课程到这里已经结束，之后可以按自己的节奏继续扩展。
